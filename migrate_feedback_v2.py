...
with app.app_context():
    try:
        db.engine.execute('ALTER TABLE buyer_feedback ADD COLUMN product_id INTEGER REFERENCES product(id)')
    except Exception as e:
        print(f'Error adding product_id: {e}')
    
    try:
        db.engine.execute('ALTER TABLE buyer_feedback ADD COLUMN farmer_id INTEGER REFERENCES users(id)')
    except Exception as e:
    ...
    for fb in feedbacks:
        if fb.order_id and not fb.farmer_id:
            ...
            if order and order.items:
                # Get farmer from the first product in the order
